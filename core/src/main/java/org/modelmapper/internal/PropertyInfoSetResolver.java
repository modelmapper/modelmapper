...
final class PropertyInfoSetResolver {
    ...
    static <T> Map<String, Accessor> resolveAccessors(T source, Class<T> type, InheritingConfiguration configuration) {
        ...
        else return resolveProperties(type, true, configuration);
    }

    static <T> Map<String, Accessor> resolveAccessorsFromValueReader(T source, InheritingConfiguration configuration, ValueReader<T> valueReader) {
...
//  }
    ...
    static <T> Map<String, Mutator> resolveMutatorsFromValueWriter(Class<T> type, InheritingConfiguration configuration, ValueWriter<T> valueWriter) {
        ...
        for (String memberName : valueWriter.memberNames(type)) {
            ...
            if (member != null)
                mutators.put(nameTransformer.transform(memberName, NameableType.GENERIC), PropertyInfoImpl.ValueWriterPropertyInfo.fromMember(member, memberName));
        }
        return mutators;
    }

    @SuppressWarnings({"unchecked"})
    private static <M extends AccessibleObject & Member, PI extends PropertyInfo> Map<String, PI> resolveProperties(Class<?> type, boolean access, Configuration configuration) {
     ...
     */
    private static <M extends AccessibleObject & Member, PI extends PropertyInfo> Map<String, PI> resolveProperties(Class<?> initialType, Class<?> type, ResolveRequest<M, PI> resolveRequest) {
        ...
        if (constructorInjector != null && constructorInjector.isApplicable(type)) {
            ...
            for (ConstructorParam member : props) {
                if (resolveRequest.namingConvention.applies(member.getName(), PropertyType.FIELD)) {
                    String name = resolveRequest.nameTransformer.transform(member.getName(), PropertyType.FIELD.equals(resolveRequest.propertyType) ? NameableType.FIELD : NameableType.METHOD);
        ...
        for (M member : resolveRequest.propertyResolver.membersFor(type)) {
            if (canAccessMember(member, resolveRequest.accessLevel) && resolveRequest.propertyResolver.isValid(member) && resolveRequest.namingConvention.applies(member.getName(), resolveRequest.propertyType)) {
                String name = resolveRequest.nameTransformer.transform(member.getName(), PropertyType.FIELD.equals(resolveRequest.propertyType) ? NameableType.FIELD : NameableType.METHOD);
                PI info = resolveRequest.propertyResolver.propertyInfoFor(initialType, member, resolveRequest.config, name);
    ...
    @SuppressWarnings("unchecked")
    private static <M extends AccessibleObject & Member, PI extends PropertyInfo> ResolveRequest<M, PI> resolveRequest(Configuration configuration, boolean access, boolean field) {
        ...
        if (field) {
            ...
            resolveRequest.propertyResolver = (PropertyInfoResolver<M, PI>) (access ? PropertyInfoResolver.ACCESSORS : PropertyInfoResolver.MUTATORS);
        }
